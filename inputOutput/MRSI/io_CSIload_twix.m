...
%     end
    ...
    MRSIStruct.geometry = geometry;
    if isfield(twix_obj.hdr.Dicom,'SoftwareVersions')
        MRSIStruct.software = [version ' ' twix_obj.hdr.Dicom.SoftwareVersions];
    else
        MRSIStruct.software = version;
    end
    MRSIStruct.PatientPosition = twix_obj.hdr.Config.PatientPosition;
    MRSIStruct.Manufacturer = 'Siemens';
    [~,filename,ext] = fileparts(twix_obj.image.filename);
    MRSIStruct.OriginalFile = [filename ext];
    % MRSIStruct.TwixHeader=twix_obj.hdr;

    MRSIStruct.nii_orientation=mrsi_orientation(twix_obj.hdr,MRSIStruct.sz(MRSIStruct.dims.t));
end
