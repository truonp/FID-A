...
function nii = io_writeniimrsi(in, outfile, UserNames, OspreyVersion)
...
if ~isfield(dims, 'kx') || dims.kx==0
    dims.kx = 0;
    dim_1  = 1;
else
    dim_1 = in.sz(dims.kx);
end
if ~isfield(dims, 'ky') || dims.ky==0
    dims.ky = 0;
    dim_2  = 1;
else
    dim_2 = in.sz(dims.ky);
end
if ~isfield(dims, 'kz')|| dims.kz==0
    dims.kz = 0;
    dim_3  = 1;
else
    dim_3 = in.sz(dims.kz);
...
dimorder = [dims.kx,dims.ky,dims.kz,in.dims.t];
dimname = {};
...
if issorted(dimorder)
    fids = reshape(in.data,ArraySize);
else
    %Permute fid non-singleton dimensions according to default BIDs precedence
    NonSingletonOrdered = permute(in.data,nonzeros(dimorder));
...
if isfield(in,'nii_mrs')
    ...
    in.nii_mrs.hdr_ext = osp_generate_nii_hdr_ext(in,'');
    in.nii_mrs.hdr     = osp_generate_nii_hdr_mrsi(in, nii.hdr,outfile);    
    nii.hdr = in.nii_mrs.hdr;
end
nii.img = conj(fids);
